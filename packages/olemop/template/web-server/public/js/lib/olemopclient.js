...
(function () {
  ...
  olemop.init = function (params, cb){
    ...
    socket.on('connect', function (){
      console.log('[olemopclient.init] websocket connected!');
      if (cb) {
        cb(socket);
      }
    });
    ...
    socket.on('message', function (data){
      ...
      if (data instanceof Array) {
        processMessageBatch(olemop, data);
      } else {
        processMessage(olemop, data);
      }
    });
  ...
  var processMessage = function (olemop, msg) {
    ...
    if (msg.id) {
      ...
      if (typeof cb !== 'function') {
        console.log('[olemopclient.processMessage] cb is not a function for request ' + msg.id);
        return;
      }
    ...
    function processCall(msg) {
      ...
      if (!!route) {
        if (!!msg.body) {
          ...
          olemop.emit(route, body);
        } else {
          olemop.emit(route,msg);
        }
      } else {
          olemop.emit(msg.body.route,msg.body);
      }
    }
  };
