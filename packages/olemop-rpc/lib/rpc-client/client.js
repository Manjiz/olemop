...
var proxyCB = function (client, serviceName, methodName, args, attach, isToSpecifiedServer) {
  ...
  if (args.length < 2) {
    logger.error('[olemop-rpc] invalid rpc invoke, arguments length less than 2, namespace: %j, serverType, %j, serviceName: %j, methodName: %j',
...
var rpcToSpecifiedServer = function (client, msg, serverType, serverId, cb) {
  ...
  if (serverId === '*') {
    ...
    if (!servers) {
      logger.error('[olemop-rpc] serverType %s servers not exist', serverType);
      return;
    }
