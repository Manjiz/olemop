var logger = require('pomelo-logger').getLogger('olemop-rpc', 'MailStation');
...
pro.dispatch = function(tracer, serverId, msg, opts, cb) {
  ...
  var send = function(tracer, err, serverId, msg, opts) {
    ...
    mailbox.send(tracer, msg, opts, function(tracer_send, send_err, args) {
      ...
      if (send_err) {
        logger.error('[olemop-rpc] fail to send message %s', send_err.stack || send_err.message);
...
var lazyConnect = function(tracer, station, serverId, factory, cb) {
  ...
  if (!server) {
    logger.error('[olemop-rpc] unknown server: %s', serverId);
    return false;
  }
  if (!online || online !== 1) {
    logger.error('[olemop-rpc] server is not online: %s', serverId);
    return false;
  }
...
var addToPending = function(tracer, station, serverId, args) {
  ...
  if (pending.length > station.pendingSize) {
    ...
    logger.warn('[olemop-rpc] station pending too much for: %s', serverId);
    return;
  }
  pending.push(args);
};
